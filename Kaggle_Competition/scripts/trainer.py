...
data=[]
from_file=np.load(path+"train_40dd.npy") #this returns an object variable
...
from_file=np.load(path+"test_40dd.npy") #this returns an object variable
for i in range(np.shape(from_file)[0]): #for every row in the object
    tmp=[]
    for j in range(np.shape(from_file)[1]): #for every column in the object. It will have only 2: inputs and outputs, but better be safe
        tmp.append(from_file[i][j]) #collect the inputs and outputs in a list for every row
...
Y_test = np.zeros((len(data), 12))#and the labels

for i in range(len(data)):#preapre in proper shape
    X_test[i,:,:,:]=np.atleast_3d(data[i][0])
    Y_test[i,:]=np.reshape(data[i][1], 12)
...
from_file=np.load(path+"valid_40dd.npy") #this returns an object variable
for i in range(np.shape(from_file)[0]): #for every row in the object
    tmp=[]
    for j in range(np.shape(from_file)[1]): #for every column in the object. It will have only 2: inputs and outputs, but better be safe
        tmp.append(from_file[i][j]) #collect the inputs and outputs in a list for every row
...
Y_valid = np.zeros((len(data), 12))#and the labels

for i in range(len(data)):#preapre in proper shape
    X_valid[i,:,:,:]=np.atleast_3d(data[i][0])
    Y_valid[i,:]=np.reshape(data[i][1], 12)
#%%
patience=20
early_stopping=EarlyStopping(patience=patience, verbose=1)

checkpointer=ModelCheckpoint(path+"model_20dd_2", save_best_only=True, verbose=1)
#%%
model= Sequential()
model.add(Conv2D(25, (5, 5), padding='same',  input_shape=(shape[0], shape[1], 1)))
model.add(Dropout(0.4))
model.add(Conv2D(22, (5, 5), padding='same'))
model.add(Dropout(0.4))
...
#%%
model = load_model(path+"model_20dd_2")
...
target_names = ["yes", "no", "up", "down", "left", "right", "on", "off", "stop", "go", "silence", "unknown"]
